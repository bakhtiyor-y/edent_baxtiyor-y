...
import { RegionModel } from 'src/app/core/models/manuals';
...
export class RegionsComponent implements OnInit {
  ...
  public selectedItems: RegionModel[];

  public editItem: RegionModel;
  ...
  add() {
    this.editItem = { id: 0 } as RegionModel;
    this.editDialog = true;
  }

  edit(item: RegionModel) {

    this.editItem = item;
    this.editDialog = true;
  }

  delete(item: RegionModel) {
    this.confirmationService.confirm({
      message: this.translate.instant('ARE_YOU_SURE_TO_DELETE_SELECTED_ENTRY'),
      header: this.translate.instant('CONFIRM'),
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.apiService.delete('api/Region?id=' + item.id)
          .toPromise()
          .then(th => {
  ...
  deleteSelected() {
    this.confirmationService.confirm({
      ...
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.apiService.post('api/Region/DeleteSelected', this.selectedItems)
          .toPromise()
          .then(th => {
  ...
  public loadData(event) {
    ...
    this.apiService.get('api/Region', params).toPromise().then(th => {
      this.items = th.data;
