...
import { InventoryOutcomeModel } from 'src/app/core/models/common';
...
@Component({
  selector: 'app-add-inventory-outcome',
  templateUrl: './add-inventory-outcome.component.html',
  styleUrls: ['./add-inventory-outcome.component.scss']
})
export class AddInventoryOutcomeComponent implements OnInit {

  @Input() public set model(item: InventoryOutcomeModel) {
    if (item) {
  ...
  save() {
    ...
    this.apiService.post('api/InventoryOutcome', outcome)
      .toPromise()
      .then(th => {
        this.saved.emit(th);
      })
  ...
  public searchRecipient(event) {
    this.apiService.get('api/User/SearchByName?name=' + event.query)
      .toPromise()
      .then(data => {
  ...
  public onRecipientSelect(user: UserModel) {
    this.editForm.get('recipientId').setValue(user.id);
  }

  public onRecipientClear() {
    this.editForm.get('recipientId').setValue('');
  }
